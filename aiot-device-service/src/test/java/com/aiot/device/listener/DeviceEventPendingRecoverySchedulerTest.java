...
class DeviceEventPendingRecoverySchedulerTest {
    ...
    private DeviceStatusStreamSubscriber subscriber;
    ...
    void setUp() {
        ...
        subscriber = mock(DeviceStatusStreamSubscriber.class);
    }
    ...
    void recoverPending_shouldClaimAndReplayWithinMaxDelivery() {
        ...
        when(streamOperations.pending("aiot:stream:device-event", "aiot-device-service-group", Range.unbounded(), 10L))
                .thenReturn(pendingMessages);

        MapRecord<String, Object, Object> claimedRecord = mock(MapRecord.class);
        when(streamOperations.claim(eq("aiot:stream:device-event"), eq("aiot-device-service-group"), eq("device-consumer-1"),
