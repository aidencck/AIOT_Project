...
| P0-02 | 配网安全 | 配网 token 仅绑定 `homeId`，可被重放或参数替换利用 | token 绑定 `homeId+productKey+deviceName`，兑换时强一致校验并一次性消费 | `aiot-device-service/src/main/java/com/aiot/device/service/impl/ProvisionServiceImpl.java`；接口：`POST /api/v1/provision/token`（推荐）、`GET /api/v1/provision/token`（兼容）、`POST /api/v1/provision/exchange` | 旧 token 重放失败；替换 `productKey/deviceName` 失败；合法流程成功 |
