...
class DeviceEventPendingRecoverySchedulerTest {
    ...
    void recoverPending_shouldClaimAndReplayWithinMaxDelivery() {
        ...
        when(streamOperations.pending("aiot:stream:device-event", "aiot-shadow-service-group", Range.unbounded(), 10L))
                .thenReturn(pendingMessages);

        MapRecord<String, Object, Object> claimedRecord = mock(MapRecord.class);
        when(streamOperations.claim(eq("aiot:stream:device-event"), eq("aiot-shadow-service-group"), eq("shadow-consumer-1"),
